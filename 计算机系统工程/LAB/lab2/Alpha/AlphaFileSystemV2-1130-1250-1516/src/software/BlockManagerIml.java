 ...
 *Version 1.0
 */
public class BlockManagerIml implements BlockManager, Serializable {
    ...
    public Block newBlock(byte[] b) {
        ...
        meta[0] = "size:"+StringOperation.ltos(b.length);
        meta[1] = "checksum:"+StringOperation.checksum(b);
        ((BlockIml)block).setData(b);
        ((BlockIml)block).setMeta(meta);
        blocks.add(block);
        return block;
    }
    ...
    public Block newEmptyBlock(int blockSize) {
        ...
        BlockIml block = new BlockIml(blockSize,this, blockId);
        ...
        String dataPath = block.getBlockDataStr();
        java.io.File dataFile = new File(dataPath);
        String metaPath = block.getBlockMetaStr();
