...
//        long newSize = (indexOfBlk+1)*blkSize;
    ...
    public void write(byte[] b){
        try {
            ...
            long oldPos = pos;
            long leave = size - (b.length + pos);/*未被新加入数据覆盖的字节数*/
            ...
            if (rest != 0) {
                ...
                byte[] lastedBlockBytes = lastedBlock == null? new byte[0]:(lastedBlock).read();
    ...
    public void setSize(long newSize) {
        ...
        try {
            if(newSize > 0 && distance < 0){
                ...
                FileUtil.writeFileMeta(getFileMetaStr(),newSize,blkSize,newSize,bytes);
            }
            else {
                ...
                FileUtil.writeFileMeta(getFileMetaStr(), newSize, blkSize, newSize, result);
                this.size = newSize;
                pos = newSize;
            }
        }catch (Exception e){
