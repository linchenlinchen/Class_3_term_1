...
if __name__ == "__main__":
    ...
    for K in tqdm(range(1, 50)):
        t = 0
        for lbd in [0.05,0.06,0.07,0.08,0.09,0.1,0.12,0.15,0.2,0.22,0.25,0.27,0.3,0.32,0.35,0.37,0.4,0.42,0.45,0.47,0.5]:
            F = np.full((K, 2967), 0.5)
            G = np.full((K, 4125), 0.5)

            while 1 == 1:
                new_F = updateF_train(lbd, G, F, train1_mat, train_u)
                new_G = updateG_train(lbd, G, new_F, train1_mat, train_i)
                dis1 = get_distance(new_F, F)
                dis2 = get_distance(new_G, G)
                # print("dis1:", dis1)
                # print("dis2:", dis2)
                if dis1 < 0.1 and dis2 < 0.1:
                    break
                F = new_F
                G = new_G
            ...
            df.to_csv("predict"+str(K)+"-"+str(t)+".csv",index=False)
            t += 1